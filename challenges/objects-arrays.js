...
for (let key in graduates) {
    universities.push(graduates[key].university);
}
universities.sort();
...
const contactInfo = [];
for (let key in graduates) {
    contactInfo.push(`${graduates[key].first_name} ${graduates[key].email}`);
}
...
const unisWithUni = [];
for (let key in graduates) {
    if (graduates[key].university.includes("Uni")) {
        unisWithUni.push(graduates[key]);
    }
}
